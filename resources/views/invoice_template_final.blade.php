    ...
    <div style="width: 100%; max-width: 100%; margin: auto;">
        ...
        <div style="margin-bottom: 0px;">
            <div style="width: 50%; float: left;">
                ...
                <p style="margin-left: 25%; font-weight: normal; line-height: 1;">
                    ...
                    @if(!empty($invoice['po_attention_to']) || !empty($invoice['first_name']) || !empty($invoice['last_name']))
                    Attention:
                    {{ $invoice['po_attention_to'] ?? ($invoice['first_name'] . ' ' . $invoice['last_name'] ?? 'N/A') }}<br>
                    @endif
            ...
            <div style="width: 50%; float: right;">
                <div
                    style="width: 100%; float: left; line-height: 1; padding-bottom:10px;padding-top:0px;padding-left:18%;">
                    <p>
                        <span style="display: block;">
                            <span
                                style="font-weight: bold; font-size: 14px; word-spacing: 0.1px; letter-spacing: 0.1px;">Invoice
                                Date</span>
                            <span
                                style="display: block; font-weight: normal; font-size: 14px;">{{ \Carbon\Carbon::parse($invoice['invoice_date'])->format('M j, Y') }}</span>
                        </span>
                        <span style="display: block; padding-top:10px;">
                            <span style="font-weight: bold; font-size: 14px;">Invoice Number</span>
                            <span
                                style="display: block; font-weight: normal; font-size: 14px;">{{ $invoice['invoice_number'] }}</span>
                        </span>
                    </p>
                </div>
                <div
                    style="padding-top:20px; width: 40%; float: right; font-weight: normal; line-height: 1; font-size: 13px">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            <thead>
                <tr>
                    <th
                        style="width:40%; font-size: 14px; text-align: left; border-bottom: 1px solid #000000; padding: 8px; font-weight: bold;">
                        Description</th>
                    <th
                        style="width=:10%; font-size: 14px; text-align: right; border-bottom: 1px solid #000000; padding: 8px; font-weight: bold;">
                        Quantity</th>
                    <th
                        style="width=15%; font-size: 14px; text-align: right; border-bottom: 1px solid #000000; padding: 8px; font-weight: bold;">
                        Unit Price</th>
                    <th
                        style="width=20%; font-size: 14px; text-align: right; border-bottom: 1px solid #000000; padding: 8px; font-weight: bold;">
                        Tax</th>
                    <th
                        style="width=15%; font-size: 14px; text-align: right; border-bottom: 1px solid #000000; padding: 8px; font-weight: bold;">
                        Amount USD</th>
            ...
            <tbody>
                @foreach ($invoice['invoice_items'] as $invoice_item)
                    <tr>
                        <td style="padding: 8px; font-size: 14px; border-bottom: 1px solid rgb(194, 187, 187);">
        ...
        <div style="margin-top: 30px;">
            <p style="font-weight: bold; font-size: 13px;">Due Date:
                {{ \Carbon\Carbon::parse($invoice['invoice_date'])->format('M j, Y') }} <br>
            ...
            <p style="margin-top: 20px; font-size: 13px;">
                ...
                ALL SALES ARE CONSIDERED FINAL. IF A REFUND IS REQUESTED WITHIN THE FIRST 24 HOURS WE WILL PROVIDE A 50%
                REFUND ONLY. ANYTHING AFTER THE 24 HOUR PERIOD THERE WILL BE NO REFUND GIVEN AND ONLY A STORE CREDIT
                WILL BE ISSUED ON A CASE BY CASE BASIS.<br><br>
                CARD HOLDER AGREES TO CHARGES TO THEIR CREDIT AND/OR DEBIT CARD AND FULLY AGREE NOT TO PURSUE A
                CHARGEBACK.
            </p>
            ...
            <footer style="position: absolute; bottom: 0; width: 100%; text-align: left; font-size: 12px; color: #555;">
                ...
                <div>
                    @php
                        ...
                        if (isset($invoice['invoice_time'])) {
                            $randomTime = \Carbon\Carbon::createFromFormat('H:i', $invoice['invoice_time']);
